...
def load_dataset(main_folder):
    ...
    # Detect markers in the image, new Aruco 4
...
def main():
    ...
    # --------------------------------------------------------------------
    #       RANDOM FOREST REGRESSION (2D ArUco -> 6D target)
    # --------------------------------------------------------------------
    if len(X) < 5:
        ...
        # "mlp-conv-vae-128" (placeholder: distinct RF hyperparams)
        ...
        #plot saver
    def save_prediction_plot(Y_test, pred_RandomForest, pred_test_vgg19,  save_path="results_plot.png"):
        """
        ...
        pred_rf_slice = pred_RandomForest[:N]
        ...
        for i, pname in enumerate(param_names):
            ...
            plt.plot(range(N), pred_rf_slice[:, i], color='orange', label='RandomForest')
    ...
    if len(images_list) < 5:
        ...
        vae.fit(images_vae, epochs=10, batch_size=8)  # just a small demo
    ...
    if len(images_list) < 5:
        ...
        X_train_img, X_test_img, Y_train_img, Y_test_img = train_test_split(
            X_imgs, Y_6d, test_size=0.1, random_state=999
        )
    ...
    save_prediction_plot(Y_test, pred_RandomForest, pred_test_vgg19,  save_path="results_plot.png")
